...
| 출력 | 기사 목록 위에 영상 재생 칸(youtube-nocookie 주소) + 영상 제목·채널·조회수. 재생하면 유튜브 공식 플레이어 스크립트로 자막을 자동으로 켠다 (한국어 → 영어 → 영어 자동 자막 → 아무 자막 순). 고를 영상이 없으면 "지금은 보여줄 영상이 없어요" 작은 칸 |
...
| **GitHub API** | 프로그램이 저장소 파일을 읽고 쓰게 해주는 GitHub 기능 | Vercel 서버는 파일을 직접 저장할 수 없어서 (설치 불필요, 배포 서버는 `DATA_REPO_TOKEN`, Actions는 자동 `GITHUB_TOKEN` 사용) | Must |
...
| 이름 | 용도 | 상태 |
|------|------|:----:|
| `DATA_REPO_TOKEN` | 데이터 저장소(my-app) 파일 읽기·쓰기 전용 세분화 토큰. 배포 서버(Vercel)는 이것만 씀 | 있음 (2026-09-18, Vercel 등록) |
| `GITHUB_TOKEN` | 관리용(내 PC). `DATA_REPO_TOKEN`이 없을 때만 앱이 씀. Vercel에는 등록하지 않음 | 있음 |
...
- [ ] 비밀 키(`VAPID_PRIVATE_KEY`, `DATA_REPO_TOKEN`, `GITHUB_TOKEN`, `OPENAI_API_KEY`, `SUBSCRIBE_PIN`)는 서버에서만 사용, 이름에 `NEXT_PUBLIC_` 붙이지 않음
